...
This is the single living document tracking everything that must happen before Thalyn is **published as an installable application** for anyone who isn't the developer. It exists because the threat model was deliberately scoped to development / personal-use; broader distribution re-opens questions that were closed under that scope.

This file is updated as Thalyn evolves. At any moment, it answers the question: *"What do we still need before shipping a binary to other people?"*

## Scope: app distribution, not source visibility

The **GitHub source repository is public** as of 2026-05-12. That decision was independent of this checklist; it gives the project free-tier CI minutes (GitHub Actions on public repos), and the source has always been intended to be open under the project's license. Reading the source on GitHub doesn't move the threat-model boundary — only **running an installer that the project distributes** does.

This checklist gates the second event, not the first. Most items here (code-signing, SBOMs, hash-chained logs, supply-chain hardening) only matter when someone other than the maintainer runs the binary.
...
This document does not block v0.x progress. It blocks the *publish a binary* button.

---

## Items

Each item links to the ADR it overrides or extends.

### Threat model
- [ ] **Re-confirm threat model.** Run a STRIDE pass with the broader audience in mind. The current threat model is scoped to development / personal-use; broader distribution requires re-confirming or updating it, including ADR-0011 (sandbox tiers).
...
- [ ] **Decide DCO vs CLA** for external code contributions. Recommend DCO.
...
- [ ] **Human-recorded delegation fixture.** The v0.33 bake-off re-run added a `delegation` fixture at [`docs/spikes/voice-bake-off/fixture/delegation.wav`](spikes/voice-bake-off/fixture/delegation.wav) to widen the WER spread on technical jargon. The audio is synthesised via Piper TTS (`en_US-hfc_female-medium`) so the numbers are reproducible from text alone, but synthetic audio is cleaner than real microphone input — record the same script with a real human voice on the public-release hardware before publishing headline WER claims. (ADR-0025; spike F2.)
...
- [ ] **Funding posture** — decide on / against GitHub Sponsors before any binary release.
...
The checklist is reviewed at every per-release architecture review. Items don't move from "to do" to "done" without a corresponding commit (and ADR if it supersedes a v1 decision).
