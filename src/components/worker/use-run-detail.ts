...
export function useRunDetail(runId: string | null): RunDetail | null {
  ...
  useEffect(() => {
    ...
    void getRun(runId)
      ...
      .catch(() => {
        // No-op outside Tauri (storybook / playwright) — stories
        // seed the detail synchronously through the static helper
        // below so the surface still has something to render.
      });
    ...
    return () => {
      active = false;
      void subStatus.then((fn) => fn()).catch(() => undefined);
      void subPlan.then((fn) => fn()).catch(() => undefined);
      void subLog.then((fn) => fn()).catch(() => undefined);
    };
  }, [runId]);
...
export function detailFromHeader(header: RunHeader): RunDetail {
  return {
    runId: header.runId,
